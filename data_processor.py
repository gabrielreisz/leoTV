...
def process_player_matches(matches, player_nickname):
    """
    Process player matches to get performance metrics.
    """
    if not matches:
        return None
    ...
    for match in matches:
        ...
        total_deaths += int(stats.get("Deaths", 1))  # Use 1 to avoid division by zero
...
def calculate_rws_leotv(player_id, match_stats_list):
    """
    Calculates the RWS-leoTV metric for a player across a list of matches, 
    baseado nas estatísticas finais da PARTIDA.
    
    Args:
        player_id (str): The player's FACEIT ID.
        match_stats_list (list): Uma lista de dicionários de estatísticas de PARTIDAS.

    Returns:
        float: A pontuação média RWS-leoTV por partida.
    """
    total_score = 0
    total_matches = 0
    ...
    for match_rounds_data in match_stats_list:
        ...
        # A FACEIT API retorna a estatística da partida como o primeiro elemento da lista 'rounds'
        if not match_rounds_data or not match_rounds_data[0].get("teams"):
            continue

        match_round_summary = match_rounds_data[0]
        
        # 1. Localizar as estatísticas do jogador na partida
        player_info = None
        player_stats = None
        ...
        # 2. Calcular a pontuação da partida
        
        # Estatísticas do jogador no match
        # Usamos .get() com valor padrão 0 para evitar KeyError caso o dado não exista
        kills = int(player_stats.get("Kills", 0))
        assists = int(player_stats.get("Assists", 0))
        damage = int(player_stats.get("Damage", 0))
        
        # Fórmula adaptada para Pontuação por PARTIDA
        # Valorizando as ações do jogador
        base_points = (kills * 5) + (assists * 3) + (damage * 0.01)

        # 3. Adicionar bônus de vitória (o foco do RWS)
        match_result = player_info.get("match_result", "0") # '1' = vitória, '0' = derrota
        
        # Multiplicamos a performance total por 1.5 se for vitória.
        if match_result == "1":
             base_points *= 1.5
